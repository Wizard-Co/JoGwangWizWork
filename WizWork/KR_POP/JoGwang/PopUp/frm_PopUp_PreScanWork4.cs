...
namespace WizWork
{
    public partial class frm_PopUp_PreScanWork4 : Form
    {
        ...
        private void setDataGrid__ChildArticle()
        {
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    {
                        ArticleIDList.Add(dr2["ArticleID"].ToString());
                    }

                    if (ArticleIDList.Distinct().Count() > 1)
                    {
                        ArticleIDCount = 2;
                    }
                    if (ArticleIDCount == 2)
                    {
                        ...
                        {
                            if (dgdMain.Rows.Count == 0) //2022-05-17 하위품이 여러개인 경우 선입선출로 처리하여 가장 먼저 생산된 라벨이 나타나게 함 || dr["ArticleID"].ToString() != dgdMain.Rows[0].Cells["ArticleID"].Value.ToString() 2022-10-12
                            {
                                dgdMain.Rows.Add(++i,
                                                         ...
                                                         x += double.Parse((Lib.CheckNull(stringFormatN0(dr["NowLoc"])))) //2021-04-06 현재고량의 합계를 보기 위해 추가
                                );
                            ...
                            else
                            {
                                //중복인 경우 그리드에 추가 안함
                                int Distinct = 0;
                                for (int num = 0; num < dgdMain.Rows.Count; num++)
                                {
                                    if (dr["ArticleID"].ToString() == dgdMain.Rows[num].Cells["ArticleID"].Value.ToString())
                                    {
                                        Distinct = 1;
                                    }
                                }

                                if (Distinct != 1) //2022-05-17 하위품이 여러개인 경우 선입선출로 처리하여 가장 먼저 생산된 라벨이 나타나게 함
                                {
                                    dgdMain.Rows.Add(++i,
                                                                Lib.CheckNull(dr["ArticleID"].ToString()),
                                                                Lib.CheckNull(dr["BuyerArticleNo"].ToString().Trim()),
                                ...
                                {
                                    //2022-05-17, 그리드 추가 안 하고 반복문 
                                }
                            }

                            BomArticleIDCount = Convert.ToInt32(dr["ChildSeq"].ToString());
                        }

                        if (dgdMain.Rows.Count != BomArticleIDCount)
                        {
                            WizCommon.Popup.MyMessageBox.ShowBox("투입 가능한 입고라벨이 부족하거나 없습니다.", "[입고내역 조회]", 0, 1);
                            btnOK.Enabled = false;
                        }
                        else
                        {
        ...
        {
            bool flag = true;
            //2021-05-31 라벨스캔 하는게 있고 안하는게 있어서 구분해서 조건 추가
            #region 지엘에스에서는 안씀
            ...
            //    || dgdMain.SelectedRows[0].Cells["Label"] == null
            //    || dgdMain.SelectedRows[0].Cells["Label"].Value.ToString().Equals("")
            //    || txtBarCodePreScan.Text == null
            //    || txtBarCodePreScan.Text == "")
            //{
            //    Message[0] = "[시작 오류]";
            //    Message[1] = "라벨을 스캔하세요.";            //"선택된 라벨이 없습니다."; 2021-05-29 문구 변경
            //    WizCommon.Popup.MyMessageBox.ShowBox(Message[1], Message[0], 0, 1);
            //    return false;
            //}
