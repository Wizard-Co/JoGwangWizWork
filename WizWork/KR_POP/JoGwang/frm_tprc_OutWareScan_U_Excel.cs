...
namespace WizWork
{
    public partial class frm_tprc_OutWareScan_U_Excel : Form
    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (chkID.Checked) //ID기준 입력
                {
                    ...
                    {
                        WizCommon.Popup.MyMessageBox.ShowBox("원자재 라벨은 예외출고에서 \r\n처리 가능합니다. \r\n 라벨번호 : " + txtBarcode.Text.ToString(), "[확인]", 0, 1);
                        txtBarcode.Text = "";
                        throw new Exception();
                    }
                }
        ...
        private void btnOrderID_Click(object sender, EventArgs e)
        {
            ...
            void PopUp_WriteTextEvent(string Custom, string InCustom, string OrderID, string BuyerArticleNo, string OrderQty, string OUnitClss, string Model, string Article, string DvlyDate, string Work, string ArticleGrp, string CustomID, string ArticleID, string OUnitClssID, string BuyerModelID, string WorkID, string InCustomID, string ArticleGrpID, string OK)
            {
                if (OK == "Cancel")
                { return; }
                else
                {
                    ...
                    txtInCustom.Text = InCustom; //매출거래처
                    txtOutCustom.Text = Custom;
                    txtOrderCustomTag.Text = CustomID;
                    txtInCustomTag.Text = InCustomID; //매출거래처ID
                    txtOutCustomTag.Text = CustomID;
                    txtBuyerArticle.Text = BuyerArticleNo;                   
                    txtArticle.Text = Article;
                    txtArticleTag.Text = ArticleID;
                    txtArticleGrp.Text = ArticleGrp;//품명그룹
                    txtArticleGrpTag.Text = ArticleGrpID;//품명그룹ID
                    txtUnitClss.Text = OUnitClss;//단위
                    txtUnitClssTag.Text = OUnitClssID;//단위ID
                }
            }
        }
        ...
        private bool CheckData()
        {
            ...
            if (txtOrderID.Text == "")
            {
                WizCommon.Popup.MyMessageBox.ShowBox("오더번호를 입력해주세요.", "[확인]", 0, 1);
                return false;
            }
