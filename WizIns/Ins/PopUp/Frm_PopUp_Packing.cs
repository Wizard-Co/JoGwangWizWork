    ...
    public partial class Frm_PopUp_Packing : Form
    {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            if (OrderArticleID.IndexOf("21", 0, 2) != -1) 
            {
                ...
                //else
                //{
                    ArticleTagID = "016"; //016 슬라이드 라벨 양식
                    if (SaveData())
                    {
        ...
        private bool LabelPrint(string PackID)
        {
            ...
            try
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    string TagID = ArticleTagID; //2021-05-10 GSL(mt_tag에서 확인)

                    list_Data.Add(Lib.CheckNull(dr["InBoxID"].ToString()));// 공정라벨

                    //재고용일 경우 빈 값처리 2022.12.02
                    if (dr["OrderFlag"].ToString() == "0")
                    {
                        list_Data.Add("");// 업체명
                    }
                    else
                    {
                        list_Data.Add(Lib.CheckNull(dr["KCustom"].ToString()));// 업체명 Order.CustomID
                    }
                    ...
                    WizWork.TSCLIB_DLL.openport(g_sPrinterName);
                    if (SendWindowDllCommand(list_Data, TagID, 1, 0)) //2021-11-29 라벨발행을 한꺼번에 처리하여 한번나오게 조건 추가
                    {
        ...
        public bool SendWindowDllCommand(List<string> vData, string sTagID, int nPrintCount, int nDefectCnt)
        {
            try
            {
                ...
                WizWork.TSCLIB_DLL.setup(stringFormatN1(strWidth), stringFormatN1(strHeight), "4", "15", "0", "4", "0"); // GLS Black Mark Setting, 2021-11-17 이걸로 수정
